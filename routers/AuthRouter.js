            ...
            client.query(`SELECT * FROM "user1"
                ...
                if (err) res.status(500).json({ success: 2, error: err});
                    ...
                    if (password === result.rows[0].password) {
                        ...
                        res.json({success: 1, userid: sessionid, redirect: `http://hedspi-library.mybluemix.net/index.html`||`http://localhost:8080/index.html`});
...
AuthRouter.delete('/logout', (req,res) => {
    req.session.destroy();
    // pool.end();
    res.send({ success: 1, message:"Logout successfully",redirect: `http://hedspi-library.mybluemix.net/index.html`||`http://localhost:8080/index.html`});
});
